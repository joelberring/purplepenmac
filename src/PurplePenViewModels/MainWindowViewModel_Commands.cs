    ...
    public partial class MainWindowViewModel
    {
        ...
        private async Task CreateCoursePdf()
        {
            ...
            while (await Services.DialogService.ShowDialogAsync(vm)) {
                ...
                PdfProductionReviewDialogViewModel productionReviewVm = new PdfProductionReviewDialogViewModel {
        ...
        /// <summary>
        /// Runs the selected profile's non-destructive colour preflight before
        /// creating a PDF. Only the confirmed capabilities of the current
        /// export path are declared, so unavailable production features are
        /// never represented as successfully exported.
        /// </summary>
        /// <param name="settings">The PDF settings selected by the user.</param>
        /// <returns>True when no selected profile blocks the export.</returns>
        private async Task<bool> ValidatePrintProfileForPdfExport(CoursePdfSettings settings)
        {
            if (String.IsNullOrEmpty(settings.PrintProfileId))
                return true;
            if (MapDisplay == null) {
                await ErrorMessage("A map must be open before a print profile can be preflighted.");
                return false;
            }
            ...
            if (!report.CanExport) {
                List<ColorPreflightRuleResult> ambiguousRules = report.RuleResults
                ...
                List<ColorPreflightRuleResult> confirmationCandidates = report.RuleResults
                    .Where(result => result.MatchStatus == ColorPreflightMatchStatus.NeedsConfirmation && !result.UserConfirmed)
                    .ToList();
                bool onlyConfirmationIsBlocking = confirmationCandidates.Count > 0
                    && report.Findings.All(finding => finding.Severity != ColorPreflightSeverity.Blocking
                        || confirmationCandidates.Any(candidate => candidate.Rule.Id == finding.RuleId));
                if (onlyConfirmationIsBlocking) {
                    StringBuilder confirmationMessage = new StringBuilder();
                    confirmationMessage.AppendFormat("The {0} print-profile preflight found partial colour matches:", profile.Name);
                    foreach (ColorPreflightRuleResult candidate in confirmationCandidates) {
                        confirmationMessage.AppendFormat("\n• {0}", candidate.Rule.Name);
                    }
                    confirmationMessage.Append("\n\nConfirm these mappings for this PDF export?");

                    if (await OKCancelMessage(confirmationMessage.ToString(), false)) {
                        foreach (ColorPreflightRuleResult candidate in confirmationCandidates) {
                            if (!settings.ConfirmedPrintProfileRuleIds.Contains(candidate.Rule.Id))
                                settings.ConfirmedPrintProfileRuleIds.Add(candidate.Rule.Id);
                        }
                        return await ValidatePrintProfileForPdfExport(settings);
                    }
                ...
                message.AppendFormat("The {0} print-profile preflight blocked PDF export:", profile.Name);
                foreach (ColorPreflightFinding finding in report.Findings) {
                    if (finding.Severity == ColorPreflightSeverity.Blocking)
                        message.AppendFormat("\n• {0}", finding.Message);
                }

                await ErrorMessage(message.ToString());
                return false;
            }

            List<ColorPreflightFinding> warnings = report.Findings
                .Where(finding => finding.Severity == ColorPreflightSeverity.Warning)
                .ToList();
            if (warnings.Count > 0) {
                StringBuilder warningMessage = new StringBuilder();
                warningMessage.AppendFormat("The {0} print-profile preflight reported warnings:", profile.Name);
                foreach (ColorPreflightFinding warning in warnings) {
                    warningMessage.AppendFormat("\n• {0}", warning.Message);
                }
                await WarningMessage(warningMessage.ToString());
            }
